...
# def get_all_dataset_links(collector_name):
#  return ["https://pch-mirror.aspath.app/resources/Routing_Data/IPv4_daily_snapshots/2021/02/route-collector.mia.pch.net/route-collector.mia.pch.net-ipv4_bgp_routes.2021.02.09.gz"]

# TODO: Refactor to add driver abstraction
def grab_new_datasets(collector_name):
    print(f"running grabber for {collector_name}")
    route_collector = (
    ...
    for link in get_all_dataset_links(collector_name):
        ...
        dataset_exists_on_db = (
            QueryBuilder().table("datasets").where({"filename": filename}).first()
        )
